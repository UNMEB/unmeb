                ...
                <div class="overflow-x-auto -mx-6">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                    ...
                                    <tr>
                                            ...
                                            <div class="flex items-center">
                                                <div class="flex-none">
                                                    <div class="w-8 h-8 rounded-[100%] ltr:mr-3 rtl:ml-3">
                                                        <img class="w-full h-full rounded-[100%] object-cover" src="{{ Avatar::create($surcharge->name)->toBase64() }}" alt="image">
                                                    </div>
                                                </div>
                                        ...
                                        <td class="table-td">
                                            {{ $surcharge->created_at->diffForHumans() }}
                                        </td>
                                        <td class="table-td">
                                            @if($surcharge->email_verified_at)
                                        ...
                                        <td class="table-td">
                                            <div class="flex space-x-3 rtl:space-x-reverse">
                                                {{--view--}}
                                                @can('user show')
                                                <a class="action-btn" href="{{ route('users.show', $surcharge) }}">
                                                    <iconify-icon icon="heroicons:eye"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{--Edit--}}
                                                @can('user update')
                                                <a class="action-btn" href="{{ route('users.edit', ['user'=>$surcharge]) }}">
                                                    <iconify-icon icon="heroicons:pencil-square"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{--delete--}}
                                                @can('user delete')
                                                <form id="deleteForm{{ $surcharge->id }}" method="POST" action="{{ route('users.destroy', $surcharge) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="action-btn cursor-pointer" onclick="sweetAlertDelete(event, 'deleteForm{{ $surcharge->id }}')" type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'users.index'" :data="$surcharges" />
                        </div>
                    </div>
                </div>
            </div>
