                ...
                <div class="overflow-x-auto -mx-6">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                    ...
                                    <tr>
                                        ...
                                        <td class="table-td">
                                            {{ $fee->course->course_code }} / {{ $fee->course->course_name }}
                                        </td>

                                        <td class="table-td">
                                            {{ $fee->surcharge->surchage_name }}
                                        </td>

                                        <td class="table-td">
                                            {{ number_format($fee->course_fee, 2) }} UGX
                                        </td>
                                        ...
                                        <td class="table-td">
                                            <div class="flex space-x-3 rtl:space-x-reverse">
                                                {{--view--}}
                                                @can('user show')
                                                <a class="action-btn" href="{{ route('users.show', $fee) }}">
                                                    <iconify-icon icon="heroicons:eye"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{--Edit--}}
                                                @can('user update')
                                                <a class="action-btn" href="{{ route('users.edit', ['user'=>$fee]) }}">
                                                    <iconify-icon icon="heroicons:pencil-square"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{--delete--}}
                                                @can('user delete')
                                                <form id="deleteForm{{ $fee->id }}" method="POST" action="{{ route('users.destroy', $fee) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="action-btn cursor-pointer" onclick="sweetAlertDelete(event, 'deleteForm{{ $fee->id }}')" type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'users.index'" :data="$fees" />
                        </div>
                    </div>
                </div>
            </div>
