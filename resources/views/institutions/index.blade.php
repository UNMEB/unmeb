                        ...
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                <thead class="bg-slate-200 dark:bg-slate-700">
                                    <tr>
                                        <th scope="col" class="table-th ">Institution Name</th>
                                        <th scope="col" class="table-th ">Location</th>
                                        <th scope="col" class="table-th ">Type</th>
                                        <th scope="col" class="table-th ">Code</th>
                                        <th scope="col" class="table-th ">Short Name</th>
                                        <th scope="col" class="table-th ">Phone Number</th>
                                        <th scope="col" class="table-th ">Box Number</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="table-td">
                                            <div class="flex space-x-3 rtl:space-x-reverse">
                                                {{--view--}}
                                                @can('user show')
                                                <a class="action-btn" href="{{ route('users.show', $institution) }}">
                                                    <iconify-icon icon="heroicons:eye"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{--Edit--}}
                                                @can('user update')
                                                <a class="action-btn" href="{{ route('users.edit', ['user'=>$institution]) }}">
                                                    <iconify-icon icon="heroicons:pencil-square"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{--delete--}}
                                                @can('user delete')
                                                <form id="deleteForm{{ $institution->id }}" method="POST" action="{{ route('users.destroy', $institution) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="action-btn cursor-pointer" onclick="sweetAlertDelete(event, 'deleteForm{{ $institution->id }}')" type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'users.index'" :data="$institutions" />
                        </div>
                    </div>
                </div>
            </div>
