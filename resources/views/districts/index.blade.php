                ...
                <div class="overflow-x-auto -mx-6">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden ">
                                    ...
                                    <tr>
                                        ...
                                        <td class="table-td">
                                            <div class="flex space-x-3 rtl:space-x-reverse">
                                                {{--view--}}
                                                @can('user show')
                                                <a class="action-btn" href="{{ route('users.show', $district) }}">
                                                    <iconify-icon icon="heroicons:eye"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{--Edit--}}
                                                @can('user update')
                                                <a class="action-btn" href="{{ route('users.edit', ['user'=>$district]) }}">
                                                    <iconify-icon icon="heroicons:pencil-square"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{--delete--}}
                                                @can('user delete')
                                                <form id="deleteForm{{ $district->id }}" method="POST" action="{{ route('users.destroy', $district) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="action-btn cursor-pointer" onclick="sweetAlertDelete(event, 'deleteForm{{ $district->id }}')" type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'users.index'" :data="$districts" />
                        </div>
                    </div>
                </div>
            </div>
